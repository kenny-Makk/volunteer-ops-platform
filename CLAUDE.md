...
S1–S11, V1 Task 1, Task 2 (ERD), and **Task 3 (SQLAlchemy models + Alembic migrations) all complete** (2026-09-05): all 12 tables from the Section 7 ERD (9 core entities + `User`, `UserRole`, `RolePermission`) now exist in a temporary dev PostgreSQL container (`volunteer-ops-postgres-dev`; formal docker-compose deferred to Task 13). `Application` modeled as the example by Claude Code; the other 11 (`Assessment`, `User`, `MembershipTier`, `Volunteer`, `Event`, `Attendance`, `Role`, `Permission`, `UserRole`, `RolePermission`, `VolunteerHour`) written by Kent and reviewed by Claude Code before migrating — recurring review catches included missing `unique` constraints on 1:1 FKs and small reference tables (`Assessment.application_id`, `User.email`, `MembershipTier.name`, `Role.name`, `Permission.name`), a composite unique constraint on `Attendance(event_id, volunteer_id)`, `Mapped[Optional[...]]` matching nullable FKs, and a `default="pending"` on `VolunteerHour.status` matching the `Application.status` pattern. Junction tables (`UserRole`, `RolePermission`) use composite primary keys, no synthetic `id`. **V1 Task 4/5 in progress** (2026-09-05): Pydantic schemas underway — `ApplicationRead`/`ApplicationCreate`/`ApplicationBase` modeled as the example by Claude Code; `VolunteerRead`/`VolunteerCreate`/`VolunteerBase` written by Kent and reviewed. **Open design note — Volunteer creation flow (parked, not forgotten, revisit at Task 6+)**: `VolunteerCreate` currently only has `name` + `membership_tier_id`, but `Volunteer.user_id` is mandatory (non-nullable) in the DB — as written, `VolunteerCreate` can't actually build a valid `Volunteer` row. This ties to FR-02 (a Volunteer should only be created from an accepted Application + passing Assessment, not free-typed by a client). Three unresolved questions, to settle together when actual Volunteer-creation business logic is written: (1) is Assessment pass/fail → Volunteer creation automatic or a manual admin action; (2) what should the API shape be — a dedicated `POST /volunteers` endpoint, or a side effect of updating an Assessment's result; (3) where does `user_id` actually come from, since `Application` itself carries no `user_id` (an applicant isn't necessarily a `User` yet at application time) — needs tracing through Application → Assessment → User to find where a User account gets linked in. Next up: finish remaining schemas, then **V1 Task 4** — first working FastAPI endpoint (`GET /volunteers`). See GitHub Issues/Projects board (not yet set up) for detailed task status going forward (Section 22 of `01_Volunteer-Ops-Platform-Spec.md` lists all tasks S1–S11, 1–15, 16–34, 35–48).
